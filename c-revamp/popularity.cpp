...
// TODO: seed with random number (do this upon generalization)
//gsl_rng_set(rand_gen, (unsigned long int) 11); // init the seed
...
int main(int argc, char* argv[]) {
    ...
    for (set<int>::iterator iter_user = data->test_users.begin(); 
        ...
        for (set<int>::iterator iter_item = data->test_items.begin(); 
            ...
            ratings.push_back(make_pair(data->popularity(item), item));
        }
    ...
    for (set<int>::iterator iter_item = data->test_items.begin(); 
        ...
        for (set<int>::iterator iter_user = data->test_users.begin(); 
            ...
            ratings.push_back(make_pair(data->popularity(item), user));
        }
        ...
        log_item(item_file, data, item, item_heldout, item_rmse, 
            item_mae, item_rank, first, item_crr, item_ncrr, item_ndcg);
    }
    fclose(item_file);
